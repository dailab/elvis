...
class ScenarioConfig:
    ...
    def from_yaml(self, yaml_str):
        ...
        self.scheduling_policy(yaml_str['scheduling_policy'])
        self.mean_park(yaml_str['mean_park'])
        self.std_deviation_park(yaml_str['std_deviation_park'])
        self.mean_soc(yaml_str['mean_soc'])
        self.std_deviation_soc(yaml_str['std_deviation_soc'])
        self.num_charging_events(yaml_str['num_charging_events'])
        self.queue_length(yaml_str['queue_length'])
        self.disconnect_by_time(yaml_str['disconnect_by_time'])
        if 'resolution_preload' in yaml_str.keys():
            if 'repeat_preload' in yaml_str.keys():
                self.transformer_preload(yaml_str['transformer_preload'],
        ...
        self.vehicle_types(vehicle_types=yaml_str['vehicle_types'])
        self.arrival_distribution(yaml_str['arrival_distribution'])

        return self

    def to_yaml(self, yaml_file):
        dictionary = dict()
        dictionary['charging_events'] = [ce.to_dict() for ce in self.arrival_distribution]
